...
typedef struct navConfig_s {
    ...
    struct {
        ...
        uint32_t geofence_radius;               // cylindrical geofence radius (m) :ST
        uint32_t geofence_height;               // cylindrical geofence height (m) :ST
        uint16_t geofence_buffer;               // minimum distance inwards from limit to re-enable geofence (m) :ST
